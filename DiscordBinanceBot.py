...
async def balance(ctx):
    # 현재 선물 USDT 잔고
    usdt = binance_bot.get_balance()
    await ctx.send(f"현재 USDT 잔고: {usdt}")

@discord_bot.command()
async def 현재포지션(ctx):
    position = binance_bot.get_futures_position()
    sign = 'Long' if position > 0 else 'Short' if position < 0 else ''
    ror = binance_bot.ror
    await ctx.send(f"현재 포지션: {position} {sign}")
    await ctx.send(f"수익률: {ror}")

@discord_bot.command()
async def set_coin(ctx, c: str):
    # 거래 코인 설정
    binance_bot.ticker = c
...
async def set_margin_type(ctx, margin_type):
    ...
    binance_bot.client.futures_change_margin_type(
        symbol=binance_bot.ticker,
...
async def 전략설정(ctx, st):
    ...
    await ctx.send(f'{binance_bot.ticker}의 선물 마진 타입이 {st}로 설정되었습니다.')
